...
// Derived from MediaPipe FACEMESH_TESSELATION (three indices per triangle).
const List<int> _faceMeshTriangleIndices = <int>[
  127,
  34,
  139,
  11,
...
List<MpFaceMeshTriangle> _buildTrianglesFromLandmarks(
  ...
  for (int i = 0; i + 2 < _faceMeshTriangleIndices.length; i += 3) {
    ...
    final int c = _faceMeshTriangleIndices[i + 2];
    if (a >= landmarks.length ||
        b >= landmarks.length ||
        c >= landmarks.length) {
      continue;
    }
